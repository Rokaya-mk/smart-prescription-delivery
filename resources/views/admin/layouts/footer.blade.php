 <footer class="footer">
                    <div class="w-100 clearfix">
                        ...
                        <span class="float-none float-sm-right mt-1 mt-sm-0 text-center">Crafted with <i class="fa fa-heart text-danger"></i> by <a href="http://MEDBOX.org/" class="text-dark" target="_blank">MEDBOX</a></span>
                    </div>
                </footer>
    ...
    <script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
        ...
        <script>
             $('#changeStatusModal').on('show.bs.modal', function (event) {
                ...
                $(this).find('#prescription_id').val(prescriptionId);
            });
            $('#sendToDeliveryModal').on('show.bs.modal', function (event) {
                var button = $(event.relatedTarget); 
                var prescriptionId = button.data('id');
                $(this).find('#prescription').val(prescriptionId);
            });
            $(document).ready(function() {
                // Initialize Select2
                $('.box-select').select2({
                    placeholder: "Select a box",
                    dropdownParent: $('#sendToDeliveryModal'),
                    width: '100%'
                });

                // Clean up Select2 when modal closes
                $('#sendToDeliveryModal').on('hidden.bs.modal', function () {
                    $('.box-select').val(null).trigger('change');
                });
            });
        </script>
        <script>
        ...
        const sidebar = document.querySelector('#sidebar'); // replace .sidebar with your sidebar class or ID
        ...
        if (direction === 'rtl') {
            ...
            if (sidebar) {
                ...
                // Or alternatively add a CSS class for RTL styling
                sidebar.classList.add('rtl-direction');
            }
        } else {
            ...
            if (sidebar) {
                ...
                sidebar.classList.remove('rtl-direction');
            }
        }
        </script>
